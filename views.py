    ...
    if request.method == 'POST':
        ...
        for file in files:
            if file and allowed_file(file.filename):
                ...
                upload_path = os.path.join(app.config['UPLOAD_FOLDER'], filenmae_n)
                file.save(upload_path)
                upload_url = '/upload/' + filenmae_n
                upload_info = {'user': 1,
                               'name': filenmae_n,
                               'url': upload_url,
                               'path': upload_path,
                ...
                n_data = dbapi.upload_get_by_url(upload_url)
...
def admin_index(error=None):
    ...
    if request == 'POST':
        site_info = {'name': request.form.get('site_name', None),
                     'title': request.form.get('public_title', None),
                     'description': request.form.get('public_description', None)}
